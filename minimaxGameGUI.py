...
class Game:
    ...
    def intro(self):
        welcome =  ''
        welcome +=  '''
        Welcome to tic-tac-toe! This game has no intelligence behind 
        ...
        for row in range(3):
            for column in range(3):
                if self.board.getBlock(row, column) is 'X':
                    ...
                    X = PhotoImage(file='X.gif')
                    self.gifBoard[row][column]= Button(self.master, image=X, command=lambda:self.updateBoard(row,column))
                    ...
                    O = PhotoImage(file='O.gif')
                    self.gifBoard[row][column]= Button(self.master, image=O, command=lambda:self.updateBoard(row,column))
